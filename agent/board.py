...
class State:
    ...
    def __init__(self):
        self.board = bytearray([CENTRE] * 64)
        self.red_tokens = 0
        self.red_stacks = 0
        self.blue_tokens = 0
        self.blue_stacks = 0
    ...
    def _apply_cascade(self, coord, direction):
        ...
        row = board_index // 8
        increment = direction.r*8 + direction.c
        ...
        while True:
            ...
            if not (0 <= board_index < 64):
                break
            if (direction.c != 0) and ((board_index // 8) != row):
                break
        ...
        for i in range(cascade_len):
            if (not (0 <= next_index < 64)) or ((direction.c != 0) and (next_index // 8 != row)):
    ...
    def _recursive_push(self, board_index, increment, row, dir_c):
        ...
        if (not (0 <= next_index < 64)) or ((dir_c != 0) and (next_index // 8 != row)):
    ...
    def legal_actions(self):
        ...
        if self.placement_count == 0:
            for i in range(64):
                legal_list.append(PlaceAction(Coord(i // 8, i % 8)))
            ...
            for i in range(64):
                ...
                r, c = i // 8, i % 8
                if is_red:
                    if (r > 0) and (self.board[i - 8] < CENTRE): continue
                    if (r < 7) and (self.board[i + 8] < CENTRE): continue
                    if (c > 0) and (self.board[i - 1] < CENTRE): continue
                    if (c < 7) and (self.board[i + 1] < CENTRE): continue
                else:
                    if (r > 0) and (self.board[i - 8] > CENTRE): continue
                    if (r < 7) and (self.board[i + 8] > CENTRE): continue
                    if (c > 0) and (self.board[i - 1] > CENTRE): continue
            ...
            is_red = self.red_turn
            directions = ((-8, Direction.Up, 0), (8, Direction.Down, 0), (-1, Direction.Left, -1), (1, Direction.Right, 1))
            ...
            for i in range(64):
                ...
                for direction in directions:
                    ...
                    if not (0 <= target_index < 64): continue
                    if (direction[2] != 0) and (target_index // 8 != r): continue
